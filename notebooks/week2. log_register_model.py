...
with mlflow.start_run(run_name="hotel-reservation-run-model",
                      tags={"git_sha": "1234567890abcd",
                            "branch": "week2"},
                            description="hotel reservation run for model logging") as run:
    ...
    model_info = mlflow.sklearn.log_model(
        sk_model=pipeline, name="lightgbm-pipeline-model", 
        signature=signature
    )
...
class HotelReservationModelWrapper(mlflow.pyfunc.PythonModel):
    ...
    def predict(self, context, model_input, threshold: float = 0.5):
...
with mlflow.start_run(tags={"branch": "week2",
    ...
    conda_env = _mlflow_conda_env(
        ...
        additional_pip_deps=[f"code/hotel_reservation-{__version__}-py3-none-any.whl",
                             ],
